...
xcrun swiftc -parse-as-library -O -module-cache-path "$WORK_DIR/module-cache-arm64" -target arm64-apple-macos26.0 -framework AppKit -framework Security -framework SwiftUI "$SOURCE" -o "$WORK_DIR/installer-arm64"
xcrun swiftc -parse-as-library -O -module-cache-path "$WORK_DIR/module-cache-x86_64" -target x86_64-apple-macos26.0 -framework AppKit -framework Security -framework SwiftUI "$SOURCE" -o "$WORK_DIR/installer-x86_64"
...
plutil -insert NSPrincipalClass -string NSApplication "$OUTPUT_APP/Contents/Info.plist"
